...
class Think extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'nav'       =>  array('attr' => 'name,field', 'close' => 1), //获取导航
    ...
    public function tagnav($tag, $content){  
        ...
        $parse  = $parse   = '<?php ';
        $parse .= '$__NAV__ = \think\Db::name(\'Channel\')->field('.$field.')->where("status=1")->order("sort")->select();';
        if($tree){
    ...
    public function tagquery($tag,$content) {
        ...
        $parseStr  =    '<?php $'.$result.' = \think\Db::name()->query("'.$sql.'");';
    ...
    public function tagvalue($tag,$content,$type='getField'){
        ...
        $parseStr   =  '<?php echo '.$filter.'(\think\Db::name("'.$name.'")';
        if(!empty($tag['table'])) {
    ...
    public function tagcount($attr,$content){
        return $this->tagvalue($attr,$content,'count');
    }
    public function tagsum($attr,$content){
        return $this->tagvalue($attr,$content,'sum');
    }
    public function tagmax($attr,$content){
        return $this->tagvalue($attr,$content,'max');
    }
    public function tagmin($attr,$content){
        return $this->tagvalue($attr,$content,'min');
    }
    public function tagavg($attr,$content){
        return $this->tagvalue($attr,$content,'avg');
    }

    public function tagdata($tag,$content){
        $name       =   !empty($tag['name'])?$tag['name']:'Document';
        $result     =   !empty($tag['result'])?$tag['result']:'article';
        $parseStr   =   '<?php $'.$result.' =\think\Db::name("'.$name.'")->alias("__DOCUMENT")';
        if(!empty($tag['table'])) {
            $parseStr .= '->table("'.$tag['table'].'")';
        }
        if(!empty($tag['where'])){
    ...
    public function tagdatalist($tag,$content) {
        ...
        $parseStr   =   '<?php $_result = \think\Db::name("'.$name.'")->alias("__DOCUMENT")';
        if(!empty($tag['table'])) {
            $parseStr .= '->table("'.$tag['table'].'")';
        }
        if(!empty($tag['where'])){
    ...
    public function tagcate($tag,$content){
        ...
        if(!empty($tag['id'])) {
            // 获取单个分类
            $parseStr   =  '<?php $'.$result.' = \think\Db::name("Category")->find('.$tag['id'].');';
            $parseStr .=  'if($'.$result.'):?>'.$content;
        }elseif(!empty($tag['name'])) {
            // 获取单个分类
            $parseStr   =  '<?php $'.$result.' = \think\Db::name("Category")->getByName('.$tag['name'].');';
            $parseStr .=  'if($'.$result.'):?>'.$content;
        }elseif(!empty($tag['pid'])){
            $key     =   !empty($tag['key'])?$tag['key']:'i';
            $mod    =   isset($tag['mod'])?$tag['mod']:'2';
            $parseStr   =  '<?php $_result = \think\Db::name("Category")->order("sort")->where("display=1 AND status=1 AND pid='.$tag['pid'].'")';
            if(!empty($tag['limit'])){
                $parseStr .= '->limit('.$tag['limit'].')';
            }
            $parseStr .= '->select();';
