...
// 检测环境是否支持可写
define('IS_WRITE', app_status !== 'sae');
// 定义当前模块路径
...
function write_config($config, $auth)
{
    if (is_array($config)) {
        ...
        //写入应用配置文件
        if (!IS_WRITE) {
            return '由于您的环境不可写，请复制下面的配置文件内容覆盖到相关的配置文件，然后再登录后台。<p>' . realpath(APP_PATH) . '/config.php</p>
            <textarea name="" style="width:650px;height:185px">' . $config_tpl . '</textarea> 
            <p>' . realpath(APP_PATH) . '/database.php</p>
            <textarea name="" style="width:650px;height:125px">' . $database . '</textarea>
            <p>' . realpath(APP_PATH) . '/tags.php</p>
            <textarea name="" style="width:650px;height:125px">' . $tags_tpl . '</textarea>';
        } else {
            if (file_put_contents(APP_PATH . 'tags.php', $tags_tpl) && file_put_contents(APP_PATH . 'config.php', $config_tpl) && file_put_contents(APP_PATH . 'database.php', $database)) {
                show_msg('配置文件写入成功');
            } else {
...
function register_administrator($db, $prefix, $admin, $auth)
{
    ...
    $sql      = str_replace(
        ...
        array($prefix, $admin['username'], $password, $admin['email'], time(), get_client_ip(1)),
        $sql);
    //执行sql
    $db->execute($sql);
